...
(function(q,n){"function"===typeof define&&define.amd?define([],n):"object"===typeof module&&module.exports?module.exports=n():q.anime=n()})(this,function(){function q(a){if(!f.col(a))try{return document.querySelectorAll(a)}catch(c){}}function n(a){return a.reduce(function(a,d){return a.concat(f.arr(d)?n(d):d)},[])}function r(a){if(f.arr(a))return a;f.str(a)&&(a=q(a)||a);return a instanceof NodeList||a instanceof HTMLCollection?[].slice.call(a):[a]}function A(a,c){return a.some(function(a){return a===c})}
function w(a){var c={},d;for(d in a)c[d]=a[d];return c}function B(a,c){var d=w(a),b;for(b in a)d[b]=c.hasOwnProperty(b)?c[b]:a[b];return d}function x(a,c){var d=w(a),b;for(b in c)d[b]=f.und(a[b])?c[b]:a[b];return d}function K(a){a=a.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i,function(a,c,d,h){return c+c+d+d+h+h});var c=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(a);a=parseInt(c[1],16);var d=parseInt(c[2],16),c=parseInt(c[3],16);return"rgb("+a+","+d+","+c+")"}function L(a){function c(a,b,c){0>
...
if(-1<a.indexOf("rotate")||-1<a.indexOf("skew"))return"deg"}function y(a,c){if(c in a.style)return getComputedStyle(a).getPropertyValue(c.replace(/([a-z])([A-Z])/g,"$1-$2").toLowerCase())||"0"}function C(a,c){if(f.dom(a)&&A(N,c))return"transform";if(f.dom(a)&&(a.getAttribute(c)||f.svg(a)&&a[c]))return"attribute";if(f.dom(a)&&"transform"!==c&&y(a,c))return"css";if(null!=a[c])return"object"}function O(a,c){var d=M(c),d=-1<c.indexOf("scale")?1:0+d;a=a.style.transform;if(!a)return d;for(var b=[],e=[],
...
case "angle":return 180*Math.atan2(g.y-e.y,g.x-e.x)/Math.PI}}function E(a,c){var d=/-?\d*\.?\d+/g;a=z(a)?a.totalLength:a;if(f.col(a))c=f.rgb(a)?a:f.hex(a)?K(a):f.hsl(a)?L(a):void 0;else{var b=t(a);a=b?a.substr(0,a.length-b.length):a;c=c?a+c:a}c+="";return{original:c,numbers:c.match(d)?c.match(d).map(Number):[0],strings:c.split(d)}}function Q(a,c){return c.reduce(function(c,b,e){return c+a[e-1]+b})}function F(a){return(a?n(f.arr(a)?a.map(r):r(a)):[]).filter(function(a,d,b){return b.indexOf(a)===d})}
function R(a){var c=F(a);return c.map(function(a,b){return{target:a,id:b,total:c.length}})}function S(a,c){var d=w(c);if(f.arr(a)){var b=a.length;2!==b||f.obj(a[0])?f.fnc(c.duration)||(d.duration=c.duration/b):a={value:a}}return r(a).map(function(a,b){b=b?0:c.delay;a=f.obj(a)&&!z(a)?a:{value:a};a.delay=a.delay||b;return a}).map(function(a){return x(a,d)})}function T(a,c,d){var b=[];a=x(a,c);for(var e in d)a.hasOwnProperty(e)||"targets"===e||b.push({name:e,tweens:S(d[e],c)});return b}function G(a,
